#include "dual_weighted_residual_obj_func3.h"
...
DualWeightedResidualObjFunc3<dim, nstate, real> :: DualWeightedResidualObjFunc3( 
...
//===================================================================================================================================================
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: compute_interpolation_matrix()
{ 
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: extract_interpolation_matrices(
    dealii::Table<2, dealii::FullMatrix<real>> &interpolation_hp)
{
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: get_projection_matrix(
...
template<int dim, int nstate, typename real>
std::vector<std::vector<dealii::types::global_dof_index>> DualWeightedResidualObjFunc3<dim, nstate, real> :: get_cellwise_dof_indices()
{
...
template<int dim, int nstate, typename real>
real DualWeightedResidualObjFunc3<dim, nstate, real> :: evaluate_functional(
    ...
    const bool compute_d2I)
{
    ...
    if(compute_dIdW || compute_dIdX || compute_d2I)
    {
        ...
        actually_compute_d2I  = false; // Hessian-vector products are evaluated directly without storing d2Is. Gauss-Newton Hession is used, which only requires first order derivatives. 
    }
...
template<int dim, int nstate, typename real>
real DualWeightedResidualObjFunc3<dim, nstate, real> :: evaluate_objective_function()
{
    dwr_error.reinit(this->dg->triangulation->n_active_cells());
    signed_cell_error.reinit(this->dg->triangulation->n_active_cells());
    dwr_error = 0;
	signed_cell_error = 0;
    ...
    for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
        ...
        signed_cell_error[cell_index] = 0.0;

        for(unsigned int i_dof = 0; i_dof < dof_indices_fine.size(); ++i_dof)
        {
            signed_cell_error[cell_index] += adjoint(dof_indices_fine[i_dof]) * residual_used(dof_indices_fine[i_dof]);
        }

		dwr_error[cell_index] = sqrt(pow(signed_cell_error[cell_index], 2) + 1.0e-8);
    } // cell loop ends
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: compute_common_vectors_and_matrices()
{
    const VectorType solution_coarse_stored = this->dg->solution;
    this->dg->change_cells_fe_degree_by_deltadegree_and_interpolate_solution(1);
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: store_dIdX()
{ 
    this->dIdX.reinit(vector_vol_nodes);
    dwr_x_Tvmult(this->dIdX, tensor2_times_dwr);
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: store_dIdW()
{
    this->dIdw.reinit(vector_coarse);
    dwr_u_Tvmult(this->dIdw, tensor2_times_dwr);
	this->dIdw.update_ghost_values();
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: d2IdWdW_vmult(
    ...
    const VectorType &in_vector) const
{
	...
	NormalVector v2(this->dg->triangulation->n_active_cells());
	tensor_matrix_squared_vmult(v2,v1);
    
    dwr_u_Tvmult(out_vector, v2);
    out_vector.update_ghost_values();
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: d2IdWdX_vmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{ 
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: d2IdWdX_Tvmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{ 
    AssertDimension(in_vector.size(), vector_coarse.size());
    ...
    dwr_x_Tvmult(out_vector, v2);
    out_vector.update_ghost_values();
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: d2IdXdX_vmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{
    AssertDimension(in_vector.size(), this->dg->high_order_grid->volume_nodes.size());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_adjoint_vmult(
    ...
    const VectorType &in_vector) const
{
    ...
    for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
        ...
        const dealii::types::global_dof_index cell_index = cell->active_cell_index();
        const std::vector<dealii::types::global_dof_index> &dof_indices_fine = cellwise_dofs_fine[cell_index];
		const real ratio_val = signed_cell_error[cell_index]/dwr_error[cell_index];

        out_vector[cell_index] = 0.0;

        for(unsigned int i_dof=0; i_dof < dof_indices_fine.size(); ++i_dof)
        {
            out_vector[cell_index] += ratio_val*residual_used(dof_indices_fine[i_dof])*in_vector(dof_indices_fine[i_dof]);
        }
    } // cell loop ends
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_residual_vmult(
    NormalVector &out_vector, 
    const VectorType &in_vector) const
{
    AssertDimension(out_vector.size(), this->dg->triangulation->n_active_cells());
    ...
    for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
        ...
        for(unsigned int i_dof=0; i_dof < dof_indices_fine.size(); ++i_dof)
        {
            out_vector[cell_index] += ratio_val*adjoint(dof_indices_fine[i_dof])*in_vector(dof_indices_fine[i_dof]);
        }
    } // cell loop ends
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_adjoint_Tvmult(
    VectorType &out_vector, 
    const NormalVector &in_vector) const
{
    ...
    for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
        ...
        for(unsigned int i_dof=0; i_dof < dof_indices_fine.size(); ++i_dof)
        {
            out_vector(dof_indices_fine[i_dof]) = in_vector[cell_index] * ratio_val*residual_used(dof_indices_fine[i_dof]);
        }
    } // cell loop ends
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_residual_Tvmult(
    VectorType &out_vector, 
    const NormalVector &in_vector) const
{
    AssertDimension(out_vector.size(), vector_fine.size());
    ...
    for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
        ...
        for(unsigned int i_dof=0; i_dof < dof_indices_fine.size(); ++i_dof)
        {
            out_vector(dof_indices_fine[i_dof]) = in_vector[cell_index] * ratio_val*adjoint(dof_indices_fine[i_dof]);
        }
    } // cell loop ends
...
//===================================================================================================================================================
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_x_vmult(
    NormalVector &out_vector,
    const VectorType &in_vector) const
{
    AssertDimension(out_vector.size(), this->dg->triangulation->n_active_cells());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_u_vmult(
    NormalVector &out_vector,
    const VectorType &in_vector) const
{
    AssertDimension(out_vector.size(), this->dg->triangulation->n_active_cells());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_x_Tvmult(
    VectorType &out_vector,
    const NormalVector &in_vector) const
{
    AssertDimension(out_vector.size(), this->dg->high_order_grid->volume_nodes.size());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: dwr_u_Tvmult(
    VectorType &out_vector,
    const NormalVector &in_vector) const
{
    AssertDimension(out_vector.size(), vector_coarse.size());
...
//===================================================================================================
//							Dot products and products with the tensor matrix
//===================================================================================================
template<int dim, int nstate, typename real>
real DualWeightedResidualObjFunc3<dim, nstate, real> :: normalvector_dot_product(
	const NormalVector &in_vector1, 
	const NormalVector &in_vector2) const
{
	AssertDimension(in_vector1.size(),this->dg->triangulation->n_active_cells()); 
	AssertDimension(in_vector2.size(),this->dg->triangulation->n_active_cells()); 
	
	const real local_dot_product = in_vector1*in_vector2;
	const real mpi_dot_product = dealii::Utilities::MPI::sum(local_dot_product, MPI_COMM_WORLD); 
	return mpi_dot_product;
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: tensor_matrix_vmult(
	NormalVector &out_vector, 
	const NormalVector &in_vector) const
{
	AssertDimension(in_vector.size(),this->dg->triangulation->n_active_cells()); 
	...
	for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
		...
		sum_val_local += in_vector[cell_index];
	}
	const real mpi_sum_val = dealii::Utilities::MPI::sum(sum_val_local, MPI_COMM_WORLD); 
	const real avg_val = mpi_sum_val/this->dg->triangulation->n_global_active_cells();
	...
	for(const auto &cell : this->dg->dof_handler.active_cell_iterators())
    {
		...
		out_vector[cell_index] = in_vector[cell_index] - avg_val;
	}
}

template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: tensor_matrix_squared_vmult(
	NormalVector &out_vector, 
	const NormalVector &in_vector) const
{
	AssertDimension(in_vector.size(),this->dg->triangulation->n_active_cells()); 
	...
	NormalVector intermediate_vector(this->dg->triangulation->n_active_cells());

	tensor_matrix_vmult(intermediate_vector, in_vector);
	tensor_matrix_vmult(out_vector, intermediate_vector);
}
...
//===================================================================================================
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: adjoint_x_vmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{
    AssertDimension(in_vector.size(), this->dg->high_order_grid->volume_nodes.size());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: adjoint_u_vmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{
    AssertDimension(in_vector.size(), vector_fine.size());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: adjoint_x_Tvmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{
    AssertDimension(in_vector.size(), vector_fine.size());
...
template<int dim, int nstate, typename real>
void DualWeightedResidualObjFunc3<dim, nstate, real> :: adjoint_u_Tvmult(
    VectorType &out_vector, 
    const VectorType &in_vector) const
{
    AssertDimension(in_vector.size(), vector_fine.size());
...
template class DualWeightedResidualObjFunc3 <PHILIP_DIM, 1, double>;
template class DualWeightedResidualObjFunc3 <PHILIP_DIM, 2, double>;
template class DualWeightedResidualObjFunc3 <PHILIP_DIM, 3, double>;
template class DualWeightedResidualObjFunc3 <PHILIP_DIM, 4, double>;
template class DualWeightedResidualObjFunc3 <PHILIP_DIM, 5, double>;
} // namespace PHiLiP
