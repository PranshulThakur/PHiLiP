...
public:
        ...
        const std::array<dealii::Tensor<1,dim,real2>,nstate> &soln_grad_at_q) const
    {
        if (boundary_id == 1001) {
            ...
            std::shared_ptr< Physics::NavierStokes<dim,dim+2,real2> > navier_stokes_physics = std::dynamic_pointer_cast<Physics::NavierStokes<dim,dim+2,real2>> (Physics::PhysicsFactory<dim,dim+2,real2>::create_Physics(this->dg->all_parameters, PDE_enum::navier_stokes, nullptr));
            ...
            if(this->dg->all_parameters->pde_type != PDE_enum::euler) {
